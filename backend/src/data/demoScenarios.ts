/**
 * Demo repair scenarios — the single source of truth for the backend.
 *
 * This data was moved verbatim out of controllers/aiController.ts so that the
 * mock AI provider can reuse it without importing the controller (which would
 * create a circular import: controller -> aiService -> mockProvider -> controller).
 *
 * The structure is UNCHANGED. aiController still re-exports `demoScenarios`,
 * so any existing importer keeps working.
 */

export interface DemoScenarioComponent {
  name: string;
  /** Absolute pixels [x, y, width, height] against DEMO_BBOX_REFERENCE. */
...
/**
 * The demo bounding boxes are absolute pixels against this reference canvas.
 * frontend/src/components/CameraFeed.tsx divides by exactly these numbers to
 * turn them into percentages, so this is the correct divisor for converting
 * demo boxes into normalized 0..1 coordinates.
 */
export const DEMO_BBOX_REFERENCE = { width: 600, height: 450 } as const;
